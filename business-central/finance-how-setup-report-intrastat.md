...
2. Open the **General** FastTab and fill the fields as necessary. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)] The table below describes some of the key fields:
   ...
   | **GST No. Based On** | Specifies based on which customer/vendor code the goods and services tax (GST) number is taken for the Intrastat report. You can choose one of the options: **Sell-to GST** or **Bill-to GST**. |
   | **Company GST No. on File** | Specifies how the company's GST registration number exports to the Intrastat file. You can choose one of the options: GST Reg. No., adding the EU Country Code as a prefix and removing the EU Country Code from GST Reg. No. |
   | **Vendor GST No. on File** | Specifies how a vendor's GST registration number is exported to the Intrastat file. You can choose one of the options: GST Reg. No., adding the EU Country Code as a prefix and removing the EU Country Code from GST Reg. No. |
   | **Customer GST No. on File** | Specifies how a customer's GST registration number is exported to the Intrastat file. You can choose one of the options: GST Reg. No., adding the EU Country Code as a prefix and removing the EU Country Code from GST Reg. No. |
   | **Get Partner GST** | Specifies which type of **Intrastat Report** line the Partner's GST registration number is updated from. Depending on your local requirements, you can choose only for receipt, only for shipment, or for both types of lines. |
3. Open the **Default Transactions** FastTab and fill the fields as necessary. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)] The table below describes some of the key fields:
   ...
   | **Default Private Person GST No.** | Specifies the default private person GST number in case the private person must have a dedicated GST number on the Intrastat report. |
   | **Default 3-Party Trade GST No.** | Specifies the default 3-party trade GST number in case you don't have its GST number. |
...
1. Choose the ![Lightbulb that opens the Tell Me feature.](media/ui-search/search_small.png "Tell me what you want to do") icon, enter **GST Reports Configuration**, then choose the related link.
2. On the **GST Reports Configuration** page, add a new line.
3. On the **GST Report Type** field, choose the **Intrastat Report** option.
4. On the **GST Report Version** field, specify the version of the report.
...
Before using a vendor in Intrastat reporting, you must have a dedicated **Country/Region Code** and **GST Registration No.** for each of them, in addition to further information on their **Vendor Card** page:
...
Before using a customer in Intrastat reporting, you must have a dedicated **Country/Region Code** and **GST Registration No.** for each of them, in addition to further information on their **Customer Card** page:
