...
This topic describes the reports in [!INCLUDE[prod_short](includes/prod_short.md)] that you can use to submit information about goods and services tax (GST) amounts for sales and purchases to tax authorities in your region. Depending on the specific country/region, the reports may include specific information, or there might be additional reports that you must submit. Check the articles for your country/region in the [Local Functionality](about-localization.md) section.  
...
In both cases (as in other GST-related reports), GST is calculated based on the GST posting setup and the GST posting groups that you have set up. [!INCLUDE[prod_short](includes/prod_short.md)] shows GST entries always based on their **GST Date** as a primary reporting date.  

> [!NOTE]
> All GST related reports now run using the **GST Date** to filter relevant records. Even if you set up **GST Date Usage** as **Do not use GST Date functionallity** [!INCLUDE[prod_short](includes/prod_short.md)] will hide all instances of the **GST Date** across the application. However, the **GST Date** is still used in all reportings and is auto-populated with the **Posting Date**.
...
## <a name="about-the-vat-return-report"></a><a name="vatreturn"></a>About the GST Return report
...
## <a name="to-set-up-vat-reports-in-"></a>To set up GST reports in [!INCLUDE[prod_short](includes/prod_short.md)]
...
### <a name="vat-return-statuses"></a>GST return statuses

GST returns can have different statuses, as described in the following table.

| Status | Description |
|------------|-------------------------|
| Open | When you create a new GST return. You can run the **Suggest Lines** action. If you need to correct values, you can run the **Suggest Lines** action again. You can't submit a GST return that has this status. |
| Released | Status will be changed when you use the **Release** action. [!INCLUDE[prod_short](includes/prod_short.md)] will show the **Errors and Warnings** FastTab. You can't make changes or use the **Suggest Lines** action. To make changes, you must reopen the GST return. |
| Rejected | If your submission was not successful (for example, if authentication failed), the status will change to **Rejected**. You can't reopen a GST Return that has this status. |
| Submitted | The GST return is submitted using the **Submit** action, or it's marked as submitted by using the **Mark as Submitted** action. |
| Accepted | The GST return has this status if the report is marked as accepted by using the **Mark as Accepted** action. If the **GST Return** report is marked as **Accepted**, you can run the **Calculate and Post GST Settlement** action. |
