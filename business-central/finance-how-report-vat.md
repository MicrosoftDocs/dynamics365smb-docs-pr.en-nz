...
| Rejected | If your submission wasn't successful (for example, if authentication failed), the status will change to **Rejected**. You can't reopen a GST Return that has this status. |
...
| Accepted | The VAT return has this status if the report is marked as accepted by using the **Mark as Accepted** action. If the **GST Return** report is marked as **Accepted**, you can run the **Calculate and Post GST Settlement** action. |
