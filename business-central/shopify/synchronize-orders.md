...
In the **Tax area priority** field, you can set the priority on how to select tax area code on addresses in order. The imported Shopify order contains information about taxes. Taxes are recalculated when you create the sales document, so it's important that the GST/tax settings are correct in [!INCLUDE[prod_short](../includes/prod_short.md)]. For more information about taxes, see [Set Up Taxes for the Shopify Connection](setup-taxes.md).
