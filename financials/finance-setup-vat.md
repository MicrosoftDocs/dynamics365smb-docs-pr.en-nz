...
You can set up GST calculations manually, but that can be tricky and time consuming. To make it easy, we've provided an assisted setup guide named **VAT Setup** that will help you with the steps. We recommend that you use the assisted setup guide to set up GST.

**Note**: You can use the guide only if you've created a My Company, and have not posted transactions that include VAT. Otherwise, it would be very easy to use different GST rates by mistake, and make GST-related reports inaccurate.  
...
* Set up GST business posting groups to define GST rates for the markets you do business in. You'll assign these to customers and vendors.  
...
You use the Import GST feature when you need to post a document where the entire amount is GST. You'll use this if you receive an invoice from the tax authorities for GST for imported goods.  
...
**Note**: This rule applies when trading with companies that are registered as VAT liable in another EU country/region. If you do business directly with consumers in other EU countries/regions, then you should contact your tax authority for applicable GST rules.  
